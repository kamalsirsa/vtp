...
bool vtTin::FindAltitudeAtPoint(const DPoint2 &p, float &fAltitude)
{
...
bool vtTin::ConvertProjection(vtProjection &proj_new)
{
...
/*
	int i, j;
	// brute force: the dumb n**2 algorithm
	for (i = 0; i < verts-1; i++)
	{
		for (j=i+1; j < verts; )
		{
			if (m_vert[i] == m_vert[j])
			{
				// merge
				_ChangeVertRefs(j, i);
				_RemoveVert(j);
...
void vtTin::_ChangeVertRefs(int from, int to)
{
	int tris = m_tri.GetSize();
	for (int i = 0; i < tris; i++)
	{
		if (m_tri[i] == from)
			m_tri[i] = to;
...
void vtTin::_RemoveVert(int kill)
{
	...
	// shift vertices down
	int verts = NumVerts();
	for (i = kill; i < verts-1; i++)
	{
		m_vert[i] = m_vert[i+1];
		m_z[i] = m_z[i+1];
	}
	m_vert.SetSize(verts-1);
	m_z.SetSize(verts-1);
}
*/

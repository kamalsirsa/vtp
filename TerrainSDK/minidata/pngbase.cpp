// (c) 2008 by Stefan Roettger and Virtual Terrain Project
...
unsigned char *decompressPNGimage(unsigned char *data,unsigned int bytes,
								  int *width,int *height,int *components)
{
	...
	if (!png_get_gAMA(png_ptr,info_ptr,&gamma))
		gamma=1.0/2.2; // assume neutral gamma
...
// compress PNG image
void compressPNGimage(unsigned char *image,int width,int height,int components,
					  unsigned char **data,unsigned int *bytes,float gamma)
{
	int i;
	...
	*data=NULL;
	...
	png_set_write_fn(png_ptr,(void *)&png_user_write_params,user_write_fn,
		user_flush_fn);
